...
def isvalid_cookie(userid, cookie):
    ...
    row = cursor.fetchone()
    if row is not None:
        return {"Status": QueryStatusString(QueryStatus.SUCCESSFULLY) if row[3] == True else QueryStatusString(QueryStatus.FAILED)}
...
def logout(userid, cookie):
    ...
    try:
        connection = getconnection()
        cursor = connection.cursor()
        query = "update cookies set status = 0 where userid = ? and cookie = ?"
        cursor.execute(query, (userid, cookie))
        connection.commit()
...
def signup(username, fullname, email, phone, password):
    ...
    try:
        ...
        cursor.execute(query_update_userinfo, (id, fullname, email, phone))
        connection.commit()
    except Exception as e:
        print(e)
        return {"Status": QueryStatusString(QueryStatus.FAILED)}
